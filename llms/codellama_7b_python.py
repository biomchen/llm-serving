...
class CodeLlamaPython7b(Configs):

    def __init__(self):
        super().__init__()
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.CODELLAMA_7B_PYTHON_MODEL
        )
        self.model = pipeline(
            self.LLAMA_TASK,
            model=self.CODELLAMA_7B_PYTHON_MODEL,
            torch_dtype=torch.float16,
            device=self.CODELLAMA_7B_PYTHON_CUDA
        )
    ...
    def get_response(self, prompt):
        ...
        responses = self.model(
            ...
            max_length=self.CODELLAMA_7B_PYTHON_MAX_LEN
        )
